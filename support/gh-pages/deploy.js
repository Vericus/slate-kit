...
async function checkLatestVersion() {
  ...
  if (!fse.pathExistsSync(latestFolder)||!fse.pathExistsSync(`${latestFolder}/version.json`)) return "0.0.0";
...
async function createVersionFile(currentVersion) {
  return await fse.writeJson(`${storybookFolder}/version.json`, {version: currentVersion});
}
...
async function tagCurrentVersion(currentVersion, latestVersion) {
  if(compareVersions(currentVersion, latestVersion) !== 1) throw new WarningError("gh pages already up to date or you forgot to update version on your main package.json");
...
async function pushToGHPages(version) {
  await exec(`cd dist && git add --all && git commit -m "${commitMsg} ${version}" && git push origin gh-pages`);
}
...
console.log(chalkProcessing("Updating github pages. This will take a moment..."));
